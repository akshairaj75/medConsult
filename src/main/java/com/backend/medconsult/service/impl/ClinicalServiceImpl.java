...
public class ClinicalServiceImpl implements ClinicalService {
    ...
    public List<LabResultListDto> getAllLabResults(CustomUserPrincipal authUser) {
        ...
        if (userEntity.getDoctor() != null) {
            ...
            labResults = labResultRepository.findByPatient_PatientId(patientId);

        } else {
            throw new RuntimeException("User is neither doctor nor patient");
        }
        return labResults.stream().map(LabResultListDto::fromEntity).toList();
    }

    @Override
    public List<LabResultListDto> getPatientResults(UUID patientId) {
        List<LabResultListDto> labResult = labResultRepository.findByPatient_PatientId(patientId).stream()
                .map(LabResultListDto::fromEntity).toList();
        return labResult;
    }

    @Override
    public LabResultDto getSingleResult(UUID resultId) {
        LabResultDto result = labResultRepository.findById(resultId).map(LabResultDto::fromEntity)
    ...
    public LabResultRegisterDto createLabResult(LabResultRegisterDto dto, List<MultipartFile> files) {
        ...
        if (dto.getLabItems() != null) {
            for (LabItemRegisterDto labItemDto : dto.getLabItems()) {
                ...
                item.setItemStatus(calculateStatus(labItemDto.getValue(), labItemDto.getReferenceMin(),
        ...
        for (LabItem item : items) {
            if (item.getItemStatus() == LabItemStatus.HIGH || item.getItemStatus() == LabItemStatus.LOW) {
                isAbnormal = true;
                break;
            }
        }
    ...
    public List<VitalsDto> getVitals(UUID patientId) {
        List<VitalsDto> vitals = vitalRepository.findByPatient_PatientId(patientId).stream().map(VitalsDto::fromEntity)
    ...
    public VitalsDto updateVitals(UUID vitalId, VitalsDto dto) {
        Vital vital = vitalRepository.findById(vitalId).orElseThrow(() -> new RuntimeException("No vitals found"));
    ...
    public List<FileDto> uploadFiles(FileUploadRequestDto dto, List<MultipartFile> files, UUID uploadedById) {
        ...
        for (MultipartFile file : files) {
            try {
                ...
                entity.setFileCategory(dto.getFileCategory() != null ? dto.getFileCategory() : FileCategory.OTHER);
    ...
    @Override
    public LabResultUpdateDto reviewLabResult(UUID labResultId, LabResultUpdateDto dto, CustomUserPrincipal authUser) {
        ...
        if (dto.getLabStatus() != null) {
            LabStatus status = dto.getLabStatus() == LabStatus.REVIEWED ? LabStatus.REVIEWED
                    : dto.getLabStatus() == LabStatus.FLAGGED ? LabStatus.FLAGGED : LabStatus.PENDING;
    ...
    private LabItemStatus calculateStatus(String value, String min, String max) {
        try {
            if (value == null || value.isBlank() || min == null || min.isBlank() || max == null || max.isBlank()) {
            ...
            return val < minVal ? LabItemStatus.LOW : val > maxVal ? LabItemStatus.HIGH : LabItemStatus.NORMAL;
